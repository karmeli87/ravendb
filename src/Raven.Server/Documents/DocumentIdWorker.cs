...
namespace Raven.Server.Documents
{
    internal unsafe class DocumentIdWorker
    {
        ...
        public static ByteStringContext<ByteStringMemoryCache>.InternalScope GetSliceFromId(ByteStringContext allocator, ReadOnlySpan<char> id, out Slice idSlice, byte? separator = null)
        {
            ...
            ByteStringContext allocator, string str, out Slice lowerIdSlice, out Slice idSlice)
        {
            ...
            Memory.Set(buffer.Ptr, 66, buffer.Length);
            ...
            // The original ID is in a lazy value string format 
            Slice.External(allocator, ptr + maxStrSize + sizeDifference, idSize, out idSlice);
            // The lower cased ID is in the raw representation, because it is used as the primary key to the Voron tables.
        ...
        private static ByteStringContext.InternalScope UnicodeGetLowerIdAndStorageKey(ByteStringContext allocator, string str,
            out Slice lowerIdSlice, out Slice idSlice, int maxStrSize, int escapePositionsSize)
        {
            ...
            var lowerSize = 0;
            var lowerPtr = buffer.Ptr + strLength * sizeof(char);
            foreach (var rune in str.EnumerateRunes())
            {
                var lower = Rune.ToLowerInvariant(rune);
                if (lower.TryEncodeToUtf8(new Span<byte>( lowerPtr + lowerSize, maxStrSize), out var read) == false)
                    throw new InvalidOperationException($"Buffer is not big enough to encode the ID '{str}'");
                lowerSize += read;
            }
            ...
            fixed (char* pChars = str)
            {
                ...
                _jsonParserState.FindEscapePositionsIn(writePos, ref idSize, escapePositionsSize);
                ...
                Slice.External(allocator, lowerPtr, lowerSize, out lowerIdSlice);
                return scope;
            }
        }
