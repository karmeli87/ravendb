...
namespace Raven.Server.Smuggler.Documents.Processors
{
    public static class BuildVersion
    {
        ...
        {
            if (buildVersion >= 70 && buildVersion < 1000)
                return BuildVersionType.GreaterThanCurrent; // debug / dev version
            if (buildVersion >= 60 && buildVersion < 70)
                return BuildVersionType.V6; // debug / dev version
            ...
            if (buildVersion >= 60000 && buildVersion <= 69999)
                return BuildVersionType.V6;
            if (buildVersion >= 70000)
